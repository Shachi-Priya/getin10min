...
function NavItem({ href, label, index, onClick }) {
  return (
    <motion.li
      custom={index}
      variants={itemVariants}
      ...
      animate="animate"
      className="group relative"
      onClick={onClick} // 👈 closes mobile menu when clicked
    >
      <Link
        href={href}
        className="inline-flex items-center text-sm md:text-base transition"
...
export default function Navbar() {
  ...
  const items = ['Home', 'Portfolio', 'Services', 'Pricing', 'Contact', 'About'];
  ...
  const nav = (
    <ul className="flex flex-col md:flex-row gap-6 items-start md:items-center">
      ...
      <motion.li
        ...
        animate="animate"
        custom={items.length}
        onClick={() => setOpen(false)} // 👈 close menu when button clicked
      >
        <Link href="/contact" className="btn btn-primary shadow-soft">
          Book a free call
        </Link>
